...
LENGTH PLAUSIBILITY: First decide whether ANYTHING in frame gives real scale (hands, boots, boards, rod, reel, cooler, deck planks, a measuring device). Set scaleReference to the specific object you used, or null if there is none. A fish alone on sand, grass, water, a boat deck with no known-size object, or held with no visible hand is NOT scale. Do not guess from "typical" size for the species - that is not scale. If scaleReference is null you MUST return null for estLengthMinIn and estLengthMaxIn. Only when scaleReference is non-null, estimate a generous plausible total-length range in inches.
...
{"commonName":"string","latinName":"string","confidence":0.0,"inRegion":true,"habitat":"inshore","catchRelease":false,"notes":"string","isPhotoOfScreen":false,"screenCheckConfidence":0.0,"scaleReference":null,"estLengthMinIn":null,"estLengthMaxIn":null,"measuringDevice":false,"deviceReadingIn":null}
    ...
    const scanToken = jwt.sign(
      {
        ...
        // Did the photo give the model anything real to scale against? With no
        // reference, its length estimate is a guess dressed as evidence, so the
        // bounds get dropped here rather than trusting a model that ignored the
        // instruction to return null. Nothing downstream may veto on a guess.
        scaleReference:    result.scaleReference ?? null,
        estLengthMinIn:    result.scaleReference ? (result.estLengthMinIn ?? null) : null,
        estLengthMaxIn:    result.scaleReference ? (result.estLengthMaxIn ?? null) : null,
...
app.post('/api/catches', async (req, res) => {
  ...
  try {
    ...
    // Length claim vs what the photo can support. This is deliberately NOT a
    // reason to void the catch: base points come off the per-species-per-day
    // ladder and have nothing to do with length, so an argument about inches
    // shouldn't cost someone the 100 they earned by catching a fish. It only
    // withholds the rewards length actually drives (the 40"/50" rings and the
    // personal-best bonus). Only fires when the photo gave real scale - with no
    // reference the model's estimate is a guess and gets no vote.
    const lengthDisputed = !!(
      scanInfo && scanInfo.scaleReference && scanInfo.estLengthMaxIn != null &&
      Number(lengthIn) > 0 && Number(lengthIn) > Number(scanInfo.estLengthMaxIn) * 1.5
    );
    if (lengthDisputed) console.log(`Length claim disputed (user ${user.id}): claimed ${lengthIn}" vs est max ${scanInfo.estLengthMaxIn}" via ${scanInfo.scaleReference}`);
    ...
    let pbBonus = null;
    if (!backdated && scanInfo && !lengthDisputed && Number(lengthIn) > 0) {
      try {
    ...
    const ringsJustEarned = backdated ? [] : await awardCatchRings(user.id, newCatch, { lengthDisputed });
...
async function awardCatchRings(userId, c, opts = {}) {
  const earned = [];
  const lengthOK = !opts.lengthDisputed;
  try {
    const owned = new Set(
    ...
    if (lengthOK && Number(c.length_in) >= 40)    await grant('sharkteethport', 'fish_40in');
    if (lengthOK && Number(c.length_in) >= 50)    await grant('jawbone',        'fish_50in');
...
async function getTideDerivedCurrent(lat, lon) {
  ...
  // GMT on both sides. Railway runs in UTC, so building "now" from server local
  // time and comparing it against lst_ldt station-local predictions read the
  // wrong tide phase by the station's UTC offset (and could miss the window at
  // day boundaries). Same GMT fix already applied to /api/tides.
  const url = `https://api.tidesandcurrents.noaa.gov/api/prod/datagetter?begin_date=${fmt(now)}&end_date=${fmt(end)}&station=${nearest.id}&product=predictions&datum=MLLW&time_zone=gmt&interval=h&units=english&application=bonetideco&format=json`;
...
// Nearest NOAA station water temp, in °F. Open-Meteo's marine model only covers
// open ocean, so inshore/estuary/inland coords get null sea_surface_temperature
// and land here. Returns null rather than throwing on any miss.
async function getNoaaWaterTemp(lat, lon) {
  const stRes = await fetch(
    'https://api.tidesandcurrents.noaa.gov/mdapi/prod/webapi/stations.json?type=waterlevels&units=english',
    { signal: AbortSignal.timeout(5000) }
  );
  const stData = await stRes.json();
  if (!stData.stations?.length) return null;
  ...
  return isNaN(tempF) ? null : Math.round(tempF * 10) / 10;
}
...
app.get('/api/marine', async (req, res) => {
  ...
  try {
    ...
    // ── No-coverage fallbacks ─────────────────────────────────────────────────
    // Open-Meteo's marine model is 8km open-ocean: inshore, estuary, and inland
    // coords come back all-null and every fallback below has to run. Chained
    // sequentially they stacked their timeouts (5s + 5s + 5s + buoy) and blew
    // past the app's 12s cap, so the client threw the whole response away and
    // rendered blanks. They're independent, so they run together and the
    // worst case is the slowest one, not the sum.
    const needCurrent = currentSpeedKts == null;
    const needTemp    = mc.sea_surface_temperature == null;
    const needWaves   = round1(mc.wave_height ?? null) == null;

    const [tideCurrent, noaaTempF, buoyData] = await Promise.all([
      needCurrent
        ? getTideDerivedCurrent(lat, lon).catch(e => { console.warn('Tide-derived current fallback failed:', e.message); return null; })
        : Promise.resolve(null),
      needTemp
        ? getNoaaWaterTemp(lat, lon).catch(e => { console.warn('NOAA water temp fallback failed:', e.message); return null; })
        : Promise.resolve(null),
      needWaves
        ? getNearestBuoyMarine(lat, lon).catch(e => { console.warn('NDBC buoy fallback failed:', e.message); return null; })
        : Promise.resolve(null),
    ]);
    ...
    const data = {
      ...
      // Open-Meteo's current model is 8km open-ocean, so inshore/estuary coords
      // get no compass bearing and these stayed blank forever. The tide-derived
      // fallback knows flood/ebb but not a bearing, so fall through to that
      // label rather than render nothing: "Outgoing" is the answer an inshore
      // angler actually wants anyway.
      currentDirectionCard: currentDirDeg != null ? degreesToCardinal(currentDirDeg) : (currentLabel ?? null),
      // Always-displayable direction. Prefer this on the client.
      currentDirectionText: currentDirDeg != null ? degreesToCardinal(currentDirDeg) : (currentLabel ?? null),
